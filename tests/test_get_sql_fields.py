...
class MyTestCase(VerboseTestCase):
    def test_first_table(self):
        expected = ['id_part_1', 'id_part_2']
        result = FirstTable.get_sql_fields()
        self.assertListEqual(expected, result)
    # end def

    def test_second_table(self):
        expected = ['cool_reference__id_part_1', 'cool_reference__id_part_2', 'additional_id']
        result = SecondTable.get_sql_fields()
        self.assertListEqual(expected, result)
    # end def

    def test_third_table(self):
        expected = ['the_reference_has_been_doubled__cool_reference__id_part_1', 'the_reference_has_been_doubled__cool_reference__id_part_2', 'the_reference_has_been_doubled__additional_id']
